...
PathIntegrator::integrateVolumeScatteringIndirect(pbr::TLState *pbrTls, const mcrt_common::Ray& ray,
        const VolumeProperties* volumeProperties, const GuideDistribution1D& densityDistribution,
        Subpixel &sp, PathVertex pv, unsigned sequenceID, float* aovs) const
{
    const bool highQualitySample = (pv.nonMirrorDepth == 0);
    const Scene *scene = MNRY_VERIFY(pbrTls->mFs->mScene);
    const Light *dummyLightPtr = scene->getLight(0);
    VolumeScatteringSampler volumeScatteringSampler(sp, pv, 1, mVolumeIndirectSamples, *dummyLightPtr,
    ...
    for (int ts = 0; ts < mVolumeIndirectSamples; ++ts) {
        ...
        scene_rdl2::math::Vec3f scatterPoint = ray.org + ray.dir * td;
        scene_rdl2::math::Vec3f scatterDir = shading::sampleSphereUniform(udl.x, udl.y);
        ...
        scene_rdl2::math::Color trToScatterPoint = vp.mTransmittance * vp.mTransmittanceH
                                                                     * exp((vp.mSigmaT + vp.mSigmaTh) * (t0 - td));
        float ph = VolumePhase(vp.mG).eval(-ray.dir, scatterDir);
        const float pdfDir = scene_rdl2::math::sOneOverFourPi;
        scene_rdl2::math::Color multiplier = (ph / (mVolumeIndirectSamples * pdfDir * pdfTd))
                                                    * vp.mSigmaS * trToScatterPoint;
        PathVertex newPv = pv;
        newPv.pathThroughput *= multiplier;
        newPv.pathPixelWeight = 0.0f;
        newPv.pathDistance += td;
