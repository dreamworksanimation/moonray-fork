...
PathIntegrator::estimateInScatteringSourceTermDirect(pbr::TLState *pbrTls, const mcrt_common::Ray& ray,
        ...
        float *aovs) const
{
    ...
    scene_rdl2::math::Vec3f scatterDir = shading::sampleSphereUniform(ul.x, ul.y);
    scene_rdl2::math::Vec3f dummy(0.0f);
    mcrt_common::RayDifferential rayIndirect(scatterPoint, scatterDir, dummy, dummy, dummy, dummy, 0.0f,
                                             scene_rdl2::math::sMaxValue, ray.getTime(), ray.getDepth()+1);
    rayIndirect.clearDifferentials();

    // Compute indirect radiance in the selected scatter direction.
    // Note: we currently set the ignoreVolumes parameter to 'true'. This limits indirect
    // in-scattering to primary rays only, in order to reduce costs.
    // We also pass in 'true' for the hitVolume parameter, as a temporary way to signal that this call to
    // computeRadianceRecurse was generated by a volume.
    scene_rdl2::math::Color Ls(0.0f);
    ...
    // between the two.
    scene_rdl2::math::Color trToSurface = vt.transmittance();

    float ph = phaseFunction.eval(-ray.dir, scatterDir);
    const float pdfDir = scene_rdl2::math::sOneOverFourPi;
    return ph * trToSurface * Ls / pdfDir;
}
        ...
        unsigned& sequenceID, bool doMIS) const
{
    ...
    scene_rdl2::math::Color trScatter = vp.mTransmittance * vp.mTransmittanceH *
        exp( (vp.mSigmaT + vp.mSigmaTh) * (vp.mTStart - tePrime) );
    ...
    scene_rdl2::math::Color Ls = estimateInScatteringSourceTermDirect(pbrTls, ray, scatterPoint,
        light, vp.mAssignmentId, VolumePhase(vp.mG), ul, ulFilter, sp, pv, sequenceID);
    return misWeight * vp.mSigmaS * trScatter * Ls / pdfTe;
}

scene_rdl2::math::Color
PathIntegrator::distanceVolumeScatteringDirect(pbr::TLState *pbrTls,
        const mcrt_common::Ray& ray, int lightIndex,
        ...
        float& deepTd, scene_rdl2::math::Color& radiance, scene_rdl2::math::Color& transmittance) const
{
    ...
    // the transmittance of this scatter point
    scene_rdl2::math::Color trScatter = vp.mTransmittance * vp.mTransmittanceH *
        exp( (vp.mSigmaT + vp.mSigmaTh) * (t0 - td) );
...
// This function largely emulates distanceVolumeScatteringDirect(). However, is is somewhat simpler since we don't
// need to do MIS between 2 sampling schemes - there's no equiangular sampling for indirect rays, since we
// are not sampling with respect to a particular light.
scene_rdl2::math::Color
PathIntegrator::distanceVolumeScatteringIndirect(pbr::TLState *pbrTls,
        ...
        float *aovs) const
{
    ...
    return vp.mSigmaS * trScatter * Ls / pdfTd;
}


scene_rdl2::math::Color
PathIntegrator::integrateVolumeScatteringDirect(pbr::TLState *pbrTls, const mcrt_common::Ray& ray,
        const VolumeProperties* volumeProperties,
        const GuideDistribution1D& densityDistribution,
        ...
        float* aovs, DeepParams* deepParams, const RayState *rs) const
{
    ...
    // we loop through _every_ light in the scene for volume scattering
    ...
    for (int lightIndex = 0; lightIndex < lightCount; ++lightIndex) {
        ...
        // no equi-angular sampling for deep volumes
        ...
        for (int ls = 0; ls < samplesPerLight; ++ls) {
            for (int ts = 0; ts < scatterSampleCount; ++ts) {
                ...
                if (doEquiAngular) {
                    ...
                    // pivot point (this is what the original paper does too)
                    float ue;
                    ...
                    scene_rdl2::math::Color contribution = invN * pv.pathThroughput *
                        ...
                        densityDistribution, sp, &pv, sequenceID, /*doMIS =*/ true);
                ...
                scene_rdl2::math::Color contribution = invN * pv.pathThroughput *
                    distanceVolumeScatteringDirect(
        ...
        float* aovs) const
{
    ...
    const Scene *scene = MNRY_VERIFY(pbrTls->mFs->mScene);
    const Light *dummyLightPtr = scene->getLight(0);
    VolumeScatteringSampler volumeScatteringSampler(sp, pv,
    ...
    for (int i = 0; i < scatterSampleCount; ++i) {
        ...
        while (volumeDepth < mMaxVolumeDepth) {
            ...
            for (int lightIndex = 0; lightIndex < lightCount; ++lightIndex) {
                ...
                scene_rdl2::math::Color scatteringSourceTerm = estimateInScatteringSourceTermDirect(
        ...
        float* surfaceT) const
{
    ...
    if (pv.volumeDepth < mMaxVolumeDepth) {
        ...
        radiance += integrateVolumeScatteringDirect(pbrTls, ray,
...
{
    // due to the floating point precision accumulated error,
    // the marching step count can still pass max step count we estimated
    // earlier slightly...in this case we'll just skip further ray marching
    // (otherwise the excess steps can result in memory corruption)
    ...
    // The only handling we can do is to ignore this case
    if (volumeRegionsCount > 0 && t1 >= sHitEpsilonEnd * sDistantLightDistance) {
        return;
    }
